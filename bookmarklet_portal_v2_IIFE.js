javascript: (function () {
    ...
    function main() {
        ...
        if (localStorage.getItem("isRunning") == "true") {
            alert("Vui lòng không chạy Boormaklet nhiều lần nếu bạn chưa tải lại trang này. Khi bạn chọn năm học hoặc học kỳ khác, GPA đã được tính và cập nhật tự động.");
            return;
        }
        else localStorage.setItem("isRunning", "true");
        ...
        // Reset the localStorage when user reload the page
        window.addEventListener("beforeunload", function (event) {
            this.localStorage.removeItem("isRunning");
        });
    }
